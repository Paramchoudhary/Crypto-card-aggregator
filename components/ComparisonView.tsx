...
const ComparisonView: React.FC<Props> = ({ cards, onRemove, onBack }) => {
  ...
  const rows = [
    ...
    { label: 'Cashback', key: 'cashbackMax', format: (val: any) => typeof val === 'number' ? `${val}%` : (val || 'N/A') },
    { label: 'Annual Fee', key: 'annualFee' },
    { label: 'FX Fee', key: 'fxFee' },
    { label: 'Staking', key: 'stakingRequired', icon: Landmark },
    ...
    { label: 'Bonus', key: 'signupBonus', icon: Coins },
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-4 sm:p-8 bg-black">
        <div className="border-4 border-white p-6 sm:p-12 max-w-lg w-full">
          ...
          <h3 className="text-xl sm:text-3xl font-black text-white uppercase mb-3 sm:mb-4">No Cards Selected</h3>
          <p className="text-white/70 font-mono mb-6 sm:mb-8 text-sm sm:text-base">Please select the cards first.</p>
          <button 
            onClick={onBack}
    ...
    <div className="flex flex-col h-full bg-black pb-16 md:pb-0">
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 md:px-8 py-4 sm:py-6 bg-black border-b-4 border-white shrink-0">
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div>
            <h2 className="text-lg sm:text-2xl md:text-3xl font-black text-white uppercase tracking-tight">Compare</h2>
            <p className="text-xs sm:text-sm text-white/50 font-mono">{cards.length} cards</p>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr>
                  <th className="p-3 sm:p-4 md:p-6 w-24 sm:w-40 md:w-64 bg-white text-black border-b-4 border-r-4 border-black sticky left-0 z-10">
                    <span className="text-[10px] sm:text-xs font-black uppercase tracking-wider">Overview</span>
                  </th>
                  {cards.map(card => (
                    <th key={card.id} className="p-3 sm:p-4 md:p-6 min-w-[160px] sm:min-w-[200px] md:min-w-[240px] border-b-4 border-l-4 border-white bg-black align-top relative group">
                      <div className="flex flex-col items-center text-center">
                        ...
                        <h3 className="font-black text-sm sm:text-base md:text-lg text-white uppercase mb-1 line-clamp-2">{card.name}</h3>
                        <span className="text-[10px] sm:text-xs font-mono px-2 py-1 border-2 border-white/50 text-white/70">{card.issuer}</span>
              ...
              <tbody className="text-xs sm:text-sm font-mono">
                  ...
                  <tr key={row.key} className={`${index % 2 === 0 ? 'bg-black' : 'bg-white/5'}`}>
                    <td className="p-2 sm:p-3 md:p-5 font-black text-white uppercase text-[10px] sm:text-xs sticky left-0 bg-inherit border-r-4 border-white z-10">
                       <div className="flex items-center gap-1 sm:gap-2">
                         {row.icon && <row.icon className="w-3 h-3 sm:w-4 sm:h-4 text-lime-400 shrink-0" />}
                    ...
                    {cards.map(card => {
                      ...
                      if (row.type === 'boolean') {
                        ...
                        displayContent = (
                          <span className={`font-mono text-[10px] sm:text-xs md:text-sm ${row.key === 'cashbackMax' ? 'text-lime-400 font-black' : 'text-white'}`}>
                      ...
                      return (
                        <td key={`${card.id}-${row.key}`} className="p-2 sm:p-3 md:p-5 text-center border-l-4 border-white/20">
                          {displayContent}
                        </td>
                      );
                    })}
                ...
                <tr className="bg-black border-t-4 border-white">
                  ...
                  {cards.map(card => (
                    <td key={`${card.id}-perks`} className="p-3 sm:p-4 md:p-6 align-top border-l-4 border-white/20">
                       <ul className="space-y-2 sm:space-y-3 text-left inline-block">
                          {card.perks.slice(0, 4).map((perk, i) => (
                             <li key={i} className="flex items-start gap-2 text-white/80">
                                ...
                                <span className="text-[10px] sm:text-xs font-mono leading-4 sm:leading-5 line-clamp-2">{perk}</span>
                             </li>
                          ))}
                          {card.perks.length > 4 && (
                            <li className="text-[10px] sm:text-xs text-white/50 font-mono">+{card.perks.length - 4} more</li>
                          )}
                       </ul>
                    </td>
                  ))}
