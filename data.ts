import { CryptoCard } from './types';

const getTwitterLogo = (handle: string) => `https://unavatar.io/twitter/${handle}`;

export const cryptoCards: CryptoCard[] = [
  {
    id: 'nexo-card',
    name: 'Nexo Card',
    issuer: 'Nexo',
    logo: getTwitterLogo('Nexo'),
    type: 'Credit',
    network: 'Mastercard',
    cashbackMax: 2,
    annualFee: 'Free',
    fxFee: '0.2%-2.5%',
    perks: ['up to 2% crypto cashback', 'earn up to 14% interest per year on unspent balance, paid out daily', 'use worldwide at over 100 million merchants', 'up to €2,000 / £1,800 free ATM withdrawals per month (Platinum tier)', 'Apple Pay and Google Pay supported', 'no monthly, annual, or inactivity card fees'],
    signupBonus: '$25 BTC',
    custody: 'Custodial',
    regions: 'EEA, UK',
    officialLink: 'https://nexo.com/crypto-card',
    cardGradient: 'from-[#1E2329] to-[#000000]',
    tierColor: 'text-blue-400',
    metal: true,
    stakingRequired: 'None (Portfolio-based)',
    atmLimit: '€2,000/mo free (Platinum)',
    mobilePay: true,
    supportedAssets: '60+ Assets',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'GBP', 'USD'],
    // rank: 1 // Gold Recommendation
  },
  {
    id: 'coinbase-card',
    name: 'Coinbase Card',
    issuer: 'Coinbase',
    logo: getTwitterLogo('coinbase'),
    type: 'Prepaid',
    network: 'Visa',
    cashbackMax: 4,
    annualFee: 'Free',
    fxFee: 'Not specified',
    perks: ['Earn crypto without buying crypto', 'No hidden fees', 'Industry-leading security features (2FA, card freezing, PIN change, etc.)', 'Switch rewards at any time', 'Fund with bank account or paycheck (zero fees)', 'No credit check', '24/7 support'],
    signupBonus: 'None',
    custody: 'Custodial',
    regions: 'US (except Hawaii)',
    officialLink: 'https://www.coinbase.com/card',
    cardGradient: 'from-blue-600 to-blue-700',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: 'Spend limits apply',
    mobilePay: true,
    supportedAssets: 'All Coinbase Assets',
    kyc: 'Required',
    supportedCurrencies: ['USD'],
    // rank: 2 // Silver Recommendation
  },
  {
    id: 'binance-card',
    name: 'Binance Card',
    issuer: 'Binance',
    logo: getTwitterLogo('binance'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 3,
    annualFee: 'Free',
    fxFee: '1-2%',
    perks: ['Earn up to 3% cashback on every purchase', 'Earn up to 250 BRL/month', 'Automatic cashback that deposits into your account every month', 'Increased Cashback for all spending in Dec 2025 - Feb 2026', 'Spend with Virtual Card Immediately After Applying', 'Pay anywhere, anytime with crypto'],
    signupBonus: '$100',
    custody: 'Custodial',
    regions: 'Brazil, LATAM',
    officialLink: 'https://www.binance.com/en/cards',
    cardGradient: 'from-[#F0B90B] to-[#F8D33A]',
    tierColor: 'text-black',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '3,000 BRL/day',
    mobilePay: true,
    supportedAssets: 'USDT, USDC, FDUSD, BNB, BTC, ETH, SOL, ADA, LINK, XRP',
    kyc: 'Required',
    supportedCurrencies: ['BRL', 'ARS', 'COP'],
    // rank: 3 // Bronze Recommendation
  },
  {
id: 'Kripicard',
name: 'Premium',
issuer: 'Kripicard',
logo: getTwitterLogo('kripicard'),
type: 'Credit',
network: 'Visa',
cashbackMax: 0,
annualFee: 'Free',
fxFee: 'Zero FX fees',
perks: [
'0% Cashback',
'0% FX fees',
'Virtual card',
'Google Pay',
'Apple Pay ',
'Paypal '
],
signupBonus: 'None',
custody: 'Self-Custody',
regions: 'Global',
officialLink: 'https://kripicard.com/register/Param25',
cardGradient: 'from-[#121214] to-[#1F1F1F]',
tierColor: 'text-orange-400',
metal: false,
stakingRequired: 'No',
atmLimit: 'NA',
mobilePay: true,
supportedAssets: 'Bitcoin, ETH, USDT, XMR, LTC, TON , ETH, SOL , BNB , DOGE , TRON ',
kyc: 'None',
supportedCurrencies: ['USD', 'HKD']
},
  {
    id: 'cdc-visa',
    name: 'Crypto.com Visa',
    issuer: 'Crypto.com',
    logo: getTwitterLogo('cryptocom'),
    type: 'Prepaid',
    network: 'Visa',
    cashbackMax: 8,
    annualFee: 'Staking',
    fxFee: 'ATM withdrawal fees, interbank exchange rates, top-up fees, and other charges may apply depending on your jurisdiction and card tier',
    perks: ['CRO Rewards on everyday spending (0% to 8% depending on tier)', 'CRO Rewards on travel spending (up to 1.5% to 15% depending on tier, coming soon)', 'Priority support (from Ruby Steel tier)', 'Voice & messaging support (from Jade Green / Royal Indigo tier)', 'Exclusive experiences: F1, UFC, UEFA & more (from Jade Green / Royal Indigo tier)', 'Private account manager (coming soon from Jade Green / Royal Indigo tier)', 'Subscription rebates: Spotify, Netflix, and Truth+ (3 months to permanent depending on tier)', 'Airport lounge access (from Jade Green / Royal Indigo tier, with variations by tier)'],
    signupBonus: '$25 CRO',
    custody: 'Custodial',
    regions: 'Global (US, EU, UK, CA, APAC)',
    officialLink: 'https://crypto.com/en/cards',
    cardGradient: 'from-slate-900 to-slate-950',
    tierColor: 'text-white',
    metal: true,
    stakingRequired: 'Yes ($500-$1,000,000 CRO)',
    atmLimit: '$200-$1,000/mo free (tier-dependent)',
    mobilePay: true,
    supportedAssets: 'Fiat/CRO',
    kyc: 'Required',
    supportedCurrencies: ['USD', 'EUR', 'GBP', 'CAD', 'SGD', 'AUD']
  },
  {
    id: 'bybit-card',
    name: 'Bybit Card',
    issuer: 'Bybit',
    logo: getTwitterLogo('Bybit_Official'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: 10,
    annualFee: 'Free',
    fxFee: '0.5%',
    perks: ['10% cashback up to 150 USD for selected users', '100% rebates on selected partners', '2% to 10% cashback rewards on everyday spending', 'Compatible with Apple Pay, Google Pay, and Samsung Pay', 'Earn up to 8% APR on idle funds with Auto-Earn', 'Free worldwide delivery'],
    signupBonus: '20 USDT',
    custody: 'Custodial',
    regions: 'EEA, LATAM',
    officialLink: 'https://www.bybit.com/en/cards',
    cardGradient: 'from-[#121214] to-[#1F1F1F]',
    tierColor: 'text-orange-400',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '100 USD/mo free',
    mobilePay: true,
    supportedAssets: '8+ Cryptos',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'USD', 'GBP']
  },
  {
    id: 'kast-card',
    name: 'KAST Card',
    issuer: 'KAST',
    logo: getTwitterLogo('KASTxyz'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 8,
    annualFee: 'Free',
    fxFee: 'Not specified',
    perks: ['Rewards points', 'Instant access', 'Points boost', 'VIP Concierge'],
    signupBonus: 'NFT',
    custody: 'Non-Custodial',
    regions: 'Global (160+ countries)',
    officialLink: 'https://www.kast.xyz/',
    cardGradient: 'from-purple-600 to-indigo-700',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'Yes (earn points boost on staked SOL)',
    atmLimit: 'Varies by Region',
    mobilePay: true,
    supportedAssets: 'USDC, SOL, ETH, BTC',
    kyc: 'Required',
    supportedCurrencies: ['USD', 'EUR', 'Global']
  }, 
  {
    id: 'gemini-card',
    name: 'Gemini Credit',
    issuer: 'Gemini',
    logo: getTwitterLogo('Gemini'),
    type: 'Credit',
    network: 'Mastercard',
    cashbackMax: 4,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['Earn up to 4% on every purchase', 'Earn bitcoin, or 50+ cryptos available on Gemini', 'Rewards are instantly deposited'],
    signupBonus: 'None',
    custody: 'Custodial',
    regions: 'US',
    officialLink: 'https://www.gemini.com/credit-card',
    cardGradient: 'from-slate-100 to-black',
    tierColor: 'text-slate-800',
    metal: true,
    stakingRequired: 'None',
    atmLimit: 'Cash Advance Fees',
    mobilePay: true,
    supportedAssets: 'Rewards in 60+',
    kyc: 'Required',
    supportedCurrencies: ['USD']
  },
  {
    id: 'metamask-card',
    name: 'MetaMask Card',
    issuer: 'ConsenSys',
    logo: getTwitterLogo('MetaMask'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: 3,
    annualFee: 'Free',
    fxFee: '1% (Metal tier 0%)',
    perks: ['Self-Custody', 'Linea Network', 'Direct Spending', 'Earn 1 MetaMask Rewards point for every $1 USD you spend', 'crypto cashback', 'exclusive benefits from our global partner network'],
    signupBonus: 'None',
    custody: 'Self-Custody',
    regions: 'EU, UK, US',
    officialLink: 'https://metamask.io/card',
    cardGradient: 'from-orange-400 to-orange-500',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: 'No fee up to $1,200/mo, then 2%',
    mobilePay: true,
    supportedAssets: 'USDC, USDT, ETH',
    kyc: 'Required',
    supportedCurrencies: ['USD', 'EUR', 'GBP']
  },
  {
    id: 'etherfi-card',
    name: 'Ether.fi Cash',
    issuer: 'Ether.fi',
    logo: getTwitterLogo('ether_fi'),
    type: 'Credit',
    network: 'Visa',
    cashbackMax: 15,
    annualFee: 'Free',
    fxFee: '1%',
    perks: ['DeFi Native', 'Yield Bearing', 'Borrow vs ETH', 'Cashback on every purchase', 'Free physical cards', 'Virtual cards', 'Contactless payment', 'Apple Pay & Google Pay', 'Non-custodial account', 'Use your crypto', 'Spend with value-accruing stables', 'Family Member Cards w/ controls (coming soon)', 'Referral rewards (1% Cashback on referral purchases + 10% points on their deposits)', 'Airport Companion lounge access', 'Concierge', 'Priority support (24/7 Live Chat and email)', 'Crypto concierge (coming soon)', 'Exclusive hotel discounts (Up to 65% off luxury hotels)', 'Price protection Up to $2K', 'Purchase protection Up to $10K', 'Extended warranty Up to $10K', 'Auto rental insurance'],
    signupBonus: 'Points',
    custody: 'Non-Custodial',
    regions: 'US',
    officialLink: 'https://www.ether.fi/cash',
    cardGradient: 'from-violet-900 to-purple-900',
    tierColor: 'text-violet-200',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '2% fee',
    mobilePay: true,
    supportedAssets: 'USDC (Yielding)',
    kyc: 'Required',
    supportedCurrencies: ['USD']
  },

  {
    id: 'wirex-card',
    name: 'Wirex Card',
    issuer: 'Wirex',
    logo: getTwitterLogo('wirexapp'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: 8,
    annualFee: 'Free',
    fxFee: 'Not specified',
    perks: ['Earn up to 8% Cryptoback™ on every purchase', 'Fee-free ATM withdrawals worldwide'],
    signupBonus: '1000 WXT',
    custody: 'Custodial',
    regions: 'Global (130+ countries)',
    officialLink: 'https://www.wirexapp.com/',
    cardGradient: 'from-[#00A58B] to-[#008670]',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'Yes (WXT)',
    atmLimit: '$400/mo Free',
    mobilePay: true,
    supportedAssets: '150+ Cryptos',
    kyc: 'Required',
    supportedCurrencies: ['USD', 'EUR', 'GBP', 'SGD', 'AUD', 'HKD', 'INR']
  },
  {
    id: 'bitpay-card',
    name: 'BitPay Card',
    issuer: 'BitPay',
    logo: getTwitterLogo('BitPay'),
    type: 'Prepaid',
    network: 'Mastercard',
    cashbackMax: 0,
    annualFee: 'Free',
    fxFee: '3%',
    perks: ['Instant Loading', 'High Limits', 'Secure'],
    signupBonus: 'None',
    custody: 'Custodial',
    regions: 'US',
    officialLink: 'https://www.bitpay.com/card',
    cardGradient: 'from-blue-800 to-blue-900',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '$2,000/day',
    mobilePay: true,
    supportedAssets: '15+ Top Caps',
    kyc: 'Required',
    supportedCurrencies: ['USD']
  },
  {
    id: 'coca-card',
    name: 'Coca Card',
    issuer: 'COCA',
    logo: getTwitterLogo('CocaWallet'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 8,
    annualFee: 'Free',
    fxFee: 'Not specified',
    perks: ['MPC Wallet', 'Non-Custodial', 'Privacy Focus', 'Up to 8% Cashback', '6% APY on balances', '50% off subscriptions'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: 'EEA, UK, Global',
    officialLink: 'https://www.coca.xyz/',
    cardGradient: 'from-pink-500 to-rose-600',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'Yes (for Tiers)',
    atmLimit: '$250/mo free',
    mobilePay: true,
    supportedAssets: 'USDT, USDC, EURC, EURS (15+ chains)',
    kyc: 'Light',
    supportedCurrencies: ['EUR', 'USD', 'GBP']
  },
  {
    id: 'brighty-card',
    name: 'Brighty Card',
    issuer: 'Brighty',
    logo: getTwitterLogo('brightyapp'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 1.75,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['Daily Interest', 'Instant Exchange', 'Digital Bank', 'Up to 10% APY on crypto', 'AI-Driven Wealth', 'Seamless Transactions', 'Quick Deposits'],
    signupBonus: 'None',
    custody: 'Custodial',
    regions: 'EEA',
    officialLink: 'https://brighty.app/',
    cardGradient: 'from-[#D1F866] to-[#A3D928]',
    tierColor: 'text-black',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '€200/mo Free',
    mobilePay: true,
    supportedAssets: 'USDT, EURT, USDC',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'USDT']
  },
{
    id: 'cypher-card',
    name: 'Cypher Card',
    issuer: 'Cypher',
    logo: getTwitterLogo('Cypher_HQ_'),
    type: 'Prepaid',
    network: 'Visa',
    cashbackMax: 'Variable',
    annualFee: '$0 (Basic with Virtual Card), $199 (Premium)',
    fxFee: '0.75%-1.75%',
    perks: ['Private Spending', 'Luxury', 'Concierge', 'Instant off ramp from crypto to fiat', 'Both physical and virtual card', 'Superior card controls', 'Load with any non-custodial wallet', 'Family spending', 'Fraud protection'],
    signupBonus: '100 $CYPR with referral code',
    custody: 'Non-Custodial',
    regions: 'Global',
    officialLink: 'https://cypherhq.io/card',
    cardGradient: 'from-neutral-900 to-black',
    tierColor: 'text-yellow-500',
    metal: true,
    stakingRequired: 'None',
    atmLimit: '$2,000/day',
    mobilePay: true,
    supportedAssets: '1000+ tokens across 15 chains',
    kyc: 'None',
    supportedCurrencies: ['USD', 'Global']
  },
  {
    id: 'venmo-card',
    name: 'Venmo Credit',
    issuer: 'Venmo',
    logo: getTwitterLogo('Venmo'),
    type: 'Credit',
    network: 'Visa',
    cashbackMax: 3,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['Auto-Purchase Crypto', 'Split Purchases', 'Social Feed'],
    signupBonus: 'None',
    custody: 'Custodial',
    regions: 'US',
    officialLink: 'https://venmo.com/about/creditcard',
    cardGradient: 'from-blue-400 to-blue-500',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '$400/day',
    mobilePay: true,
    supportedAssets: 'BTC, ETH, LTC, BCH',
    kyc: 'Required',
    supportedCurrencies: ['USD']
  },
  {
    id: 'bitrefill-card',
    name: 'Bitrefill Card',
    issuer: 'Bitrefill',
    logo: getTwitterLogo('Bitrefill'),
    type: 'Prepaid',
    network: 'Visa',
    cashbackMax: 0,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['No monthly fee', 'No issuing fee', 'Virtual & Physical Card', 'Unlimited SEPA Withdrawal Limit', '24/7 support', 'Withdraw cash at ATMs globally'],
    signupBonus: '-',
    custody: 'Custodial',
    regions: 'Europe',
    officialLink: 'https://t.co/UQGED9yISR' ,
    cardGradient: 'from-red-500 to-red-200' , 
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '€350/day & €3,000/month',
    mobilePay: true,
    supportedAssets: 'USDC, BTC, ETH, BNB, Polygon, Solana',
    kyc: 'Required',
    supportedCurrencies: ['USD',]
  },
  {
    id: 'whitebit-card',
    name: 'WhiteBIT Card',
    issuer: 'WhiteBIT',
    logo: getTwitterLogo('WhiteBit'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 10,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['WBIT Holder Perks', 'Fast Verification', 'Secure', 'up to 10% cashback in BTC', 'zero fees for top-up', 'contactless payments', 'earn cashback in selected categories', 'referral program'],
    signupBonus: 'None',
    custody: 'Custodial',
    regions: 'EEA',
    officialLink: 'https://whitebit.com/crypto-card',
    cardGradient: 'from-yellow-400 to-orange-400',
    tierColor: 'text-black',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '€2,500/mo',
    mobilePay: true,
    supportedAssets: 'WBT, USDT, BTC, ETH, XRP, SOL, NEAR, ADA, AVAX, EUR, DOGE',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'USD', 'UAH']
  },
  {
    id: 'wayex-card',
    name: 'Wayex Card',
    issuer: 'Wayex',
    logo: getTwitterLogo('WayexApp'),
    type: 'Prepaid',
    network: 'Visa',
    cashbackMax: 1,
    annualFee: 'Free',
    fxFee: '0% on USD',
    perks: ['Crypto to Fiat', 'Instant', 'User Friendly', 'Automatic yield (up to 5% everyday, up to 20% fixed-term)', 'instant swaps', 'global USD/EUR accounts'],
    signupBonus: 'None',
    custody: 'Self-Custody',
    regions: 'US, Global',
    officialLink: 'https://www.wayex.com/en-US',
    cardGradient: 'from-green-400 to-emerald-500',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: 'Not specified',
    mobilePay: true,
    supportedAssets: 'Stablecoins (USDC, USDT)',
    kyc: 'Required',
    supportedCurrencies: ['USD', 'EUR']
  },
  {
    id: 'gnosis-card',
    name: 'Gnosis Pay',
    issuer: 'Gnosis',
    logo: getTwitterLogo('gnosispay'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 4,
    annualFee: '€30',
    fxFee: 'Optional',
    perks: ['On-chain', 'Self-Custody', 'Developer Friendly', 'Instant Deployment', 'Multi-Currency Native', 'Self-Custodial', 'Global Coverage'],
    signupBonus: 'None',
    custody: 'Self-Custody',
    regions: 'UK, EEA',
    officialLink: 'https://gnosispay.com/',
    cardGradient: 'from-[#058c49] to-[#046c38]',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '€500/day',
    mobilePay: true,
    supportedAssets: 'EURe, GNO, ETH, USDC, EURC',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'GBP']
  },
  {
    id: 'fold-card',
    name: 'Fold Card',
    issuer: 'Fold',
    logo: getTwitterLogo('fold_app'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 15,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['Bitcoin Rewards', 'Spin Wheel', 'Bill Pay', 'Buy and sell bitcoin with zero fees', 'Up to 1.5% back with ACH bill pay rewards', 'Gift card rewards and up to 15% back on merchant boosts', '1.5% back on categories like dining and travel'],
    signupBonus: 'Free Sats',
    custody: 'Custodial',
    regions: 'US',
    officialLink: 'https://foldapp.com/',
    cardGradient: 'from-[#FFD700] to-[#E6C200]',
    tierColor: 'text-black',
    metal: false,
    stakingRequired: 'None',
    atmLimit: 'No Fees (In-Network)',
    mobilePay: true,
    supportedAssets: 'Bitcoin Only',
    kyc: 'Required',
    supportedCurrencies: ['USD']
  },
  {
    id: 'bleap-card',
    name: 'Bleap Card',
    issuer: 'Bleap',
    logo: getTwitterLogo('Bleap_finance'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: 2,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['Account Abstraction', 'Gasless', 'Easy Setup', 'Instant crypto-to-fiat conversion', 'free ATM withdrawals up to $400', 'Apple Pay/Google Pay support'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: 'EEA',
    officialLink: 'https://www.bleap.finance/',
    cardGradient: 'from-[#6366f1] to-[#4f46e5]',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '€400/mo free',
    mobilePay: true,
    supportedAssets: 'USDC, ETH',
    kyc: 'Required',
    supportedCurrencies: ['EUR']
  },
  {
    id: 'bitpanda-card',
    name: 'Bitpanda Card',
    issuer: 'Bitpanda',
    logo: getTwitterLogo('bitpanda'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 1,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['Spend Assets', 'Metals', 'Stocks', 'Free card', 'No monthly costs', '0% FX fees for non-Euro transactions', 'In-app card management', 'Apple Pay support', 'Real-time spending tracking', 'Visa security & fraud prevention'],
    signupBonus: 'None',
    custody: 'Custodial',
    regions: 'EEA',
    officialLink: 'https://www.bitpanda.com/en/card',
    cardGradient: 'from-[#23d186] to-[#1a9f66]',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'Yes (BEST)',
    atmLimit: '€500/day',
    mobilePay: true,
    supportedAssets: 'All Bitpanda Assets',
    kyc: 'Required',
    supportedCurrencies: ['EUR']
  },
  {
    id: 'zypto-card',
    name: 'Zypto Card',
    issuer: 'Zypto',
    logo: getTwitterLogo('ZyptoApp'),
    type: 'Prepaid',
    network: 'Visa',
    cashbackMax: 0,
    annualFee: 'Free',
    fxFee: '2%',
    perks: ['Virtual Cards', 'Gift Cards', 'Wide Crypto Support', 'Use anywhere Visa is accepted', 'Up to $1M in spending power', 'Fast KYC', 'Instant funding', 'Premium VISA benefits', 'Personal concierge', 'Travel and lifestyle benefits'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: 'Global',
    officialLink: 'https://zypto.com/',
    cardGradient: 'from-gray-800 to-gray-900',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: 'Varies',
    mobilePay: true,
    supportedAssets: '100+ cryptocurrencies',
    kyc: 'Light',
    supportedCurrencies: ['USD', 'EUR', 'Global']
  },
  {
    id: 'ledger-card',
    name: 'Ledger CL',
    issuer: 'Ledger',
    logo: getTwitterLogo('Ledger'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 1,
    annualFee: 'Free',
    fxFee: '1%',
    perks: ['Ledger Live', 'Secured by Ledger', 'Collateral', 'Earn 1% cashback crypto rewards', 'Securely top up your card with crypto', 'Withdraw cash at ATMs', 'Unlock liquidity while holding onto your assets'],
    signupBonus: 'None',
    custody: 'Self-Custody',
    regions: 'UK, EEA',
    officialLink: 'https://shop.ledger.com/pages/cl-card-crypto-card',
    cardGradient: 'from-[#000000] to-[#1a1a1a]',
    tierColor: 'text-[#bc5603]',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '€600/mo',
    mobilePay: true,
    supportedAssets: 'BTC, ETH, USDT',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'GBP', 'USD']
  },
  {
    id: 'tuyo-card',
    name: 'Tuyo Card',
    issuer: 'Tuyo',
    logo: getTwitterLogo('tuyo_io'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: 0,
    annualFee: 'Free',
    fxFee: 'up to 1% for non-USD',
    perks: ['Latin America', 'Easy Onramp', 'Mobile First', 'Keeping custody (self-custodial)', 'Privacy of transaction details', 'Compatibility with USDC on Base', 'Support for mobile wallets (Apple Pay and Google Pay)'],
    signupBonus: 'None',
    custody: 'Self-Custody',
    regions: 'US, EEA, LATAM',
    officialLink: 'https://tuyo.com/card',
    cardGradient: 'from-fuchsia-600 to-purple-600',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: 'Not supported',
    mobilePay: true,
    supportedAssets: 'USDC, USDT',
    kyc: 'None',
    supportedCurrencies: ['USD', 'EUR']
  },
  {
    id: 'redotpay-card',
    name: 'RedotPay Card',
    issuer: 'RedotPay',
    logo: getTwitterLogo('RedotPay'),
    type: 'Prepaid',
    network: 'Visa',
    cashbackMax: 0,
    annualFee: 'Free',
    fxFee: '1.2%',
    perks: ['Hong Kong', 'Virtual Card', 'Fast KYC', 'Shop, withdraw, and manage your funds globally', 'No Annual Fees', 'Simply tap or swipe to pay', 'Withdraw cash at ATMs globally'],
    signupBonus: '$5',
    custody: 'Custodial',
    regions: 'Global (excluding US/Sanctioned)',
    officialLink: 'https://www.redotpay.com/',
    cardGradient: 'from-red-600 to-red-700',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '$500/day',
    mobilePay: true,
    supportedAssets: 'USDT, USDC, BTC, ETH',
    kyc: 'Light',
    supportedCurrencies: ['USD', 'HKD', 'Global']
  },
  {
    id: 'tria-card',
    name: 'Tria Card',
    issuer: 'Tria',
    logo: getTwitterLogo('useTria'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 5,
    annualFee: 'Free',
    fxFee: '0.5%',
    perks: ['Chain Abstraction', 'Any Token', 'Gasless', 'Boosted Rewards with Tria Card', 'Activate higher yields and exclusive opportunities when you hold and use a Tria Card'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: 'Global',
    officialLink: 'https://www.tria.so/en',
    cardGradient: 'from-violet-600 to-fuchsia-600',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: 'Standard',
    mobilePay: true,
    supportedAssets: 'BTC, Sol, and other 1000+ coins',
    kyc: 'Required',
    supportedCurrencies: ['USD', 'Global']
  },
  {
    id: 'avici-card',
    name: 'Avici Card',
    issuer: 'Avici',
    logo: getTwitterLogo('AviciMoney'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 2,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['DeFi Integration', 'Yield', 'Premium Design', '1.5%–2% cashback on eligible purchases', 'Instant virtual & physical cards'],
    signupBonus: 'Waitlist',
    custody: 'Non-Custodial',
    regions: 'EEA',
    officialLink: 'https://avici.money/',
    cardGradient: 'from-[#000000] to-[#222]',
    tierColor: 'text-white',
    metal: true,
    stakingRequired: 'None',
    atmLimit: '€1,000/day',
    mobilePay: true,
    supportedAssets: 'ERC-20 Tokens',
    kyc: 'Required',
    supportedCurrencies: ['EUR']
  },
  {
    id: 'oobit-card',
    name: 'Oobit Card',
    issuer: 'Oobit',
    logo: getTwitterLogo('Oobit'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 6,
    annualFee: 'Free',
    fxFee: '1%',
    perks: ['Tap to Pay', 'Instant Convert', 'Rewards', 'A crypto card that lets you use USDT, Bitcoin, Ethereum as cash', 'Spend instantly at millions of stores'],
    signupBonus: '10 OBT',
    custody: 'Non-Custodial',
    regions: 'EEA, USA, South Africa, Brazil',
    officialLink: 'https://www.oobit.com/',
    cardGradient: 'from-zinc-800 to-zinc-900',
    tierColor: 'text-lime-400',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '€500/day',
    mobilePay: true,
    supportedAssets: 'BTC, ETH, OBT, USDT',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'USD', 'BRL']
  },

   {
    id: 'plutus-card',
    name: 'Plutus Card',
    issuer: 'Plutus',
    logo: getTwitterLogo('plutus'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 9,
    annualFee: 'Free',
    fxFee: '0%',
    perks: ['Perks Selection', 'Stacking', 'Subscription', 'Earn 3-9% in PLUS Rewards on card spending', 'enjoy £10 in rewards back monthly at 50+ Perks'],
    signupBonus: '$10 PLU',
    custody: 'Non-Custodial',
    regions: 'UK, EEA',
    officialLink: 'https://www.plutus.it/',
    cardGradient: 'from-blue-700 to-indigo-800',
    tierColor: 'text-blue-200',
    metal: false,
    stakingRequired: 'Yes (PLU)',
    atmLimit: '£500/day',
    mobilePay: true,
    supportedAssets: 'PLU, ETH, BTC',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'GBP']
  },
  {
    id: 'deblock-card',
    name: 'Deblock Card',
    issuer: 'Deblock',
    logo: getTwitterLogo('deblockapp'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: 1,
    annualFee: 'Free standard plan',
    fxFee: '0%',
    perks: ['Europe', 'Virtual Card', 'Self Custodial', 'MiCA licensed', 'Fast KYC', 'Deblock is: IBAN, cards, instant bank transfers, currency and Crypto exchanges... For free and with no bank charges!', 'No Annual Fees', 'Custom NFT branded cards', 'Withdraw cash at ATMs globally'],
    signupBonus: '3 months Premium Plan',
    custody: 'Non-Custodial',
    regions: 'EEA',
    officialLink: 'https://deblock.com/f-cca',
    cardGradient: 'from-red-600 to-red-700',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '€30,000/monthly',
    mobilePay: true,
    supportedAssets: 'BTC, ETH, USDT, USDC, SOL, BNB, PENGU, HYPE, TON, and hundreds more',
    kyc: 'Required',
    supportedCurrencies: ['EUR']
  },
  {
  id: 'ready-lite',
  name: 'Ready Lite',
  issuer: 'Ready',
  logo: getTwitterLogo('Ready'),
  type: 'Debit',
  network: 'Mastercard',
  cashbackMax: 0.5,
  annualFee: 'Free',
  fxFee: 'Zero FX fees',
  perks: [
    '0.5% Cashback',
    '0% FX fees',
    'Virtual card',
    'Google Pay',
    'Apple Pay coming soon'
  ],
  signupBonus: 'None',
  custody: 'Self-Custody',
  regions: 'Global',
  officialLink: 'https://t.co/uXiZjnByAs',
cardGradient: 'from-[#F36A3D] to-[#FF875B]',
  tierColor: 'text-black',
  metal: false,
  stakingRequired: 'No',
  atmLimit: '$200/month free',
  mobilePay: true,
  supportedAssets: 'Bitcoin, ETH, USDC',
  kyc: 'Required',
  supportedCurrencies: ['USD']
},
  {
    id: 'tapx-card',
    name: 'TapX Card',
    issuer: 'TapX',
    logo: getTwitterLogo('TapxCard_'),
    type: 'Debit',
    network: 'Visa/Mastercard',
    cashbackMax: null,
    annualFee: 'Free',
    fxFee: '0% FX fees',
    perks: [
      'Cashback coming soon',
      '0% FX fees',
      'Physical & virtual card',
      'Google Pay',
      'Apple Pay'
    ],
    signupBonus: 'None',
    custody: 'Self-Custody',
    regions: 'Global',
    officialLink: 'https://tapxcard.com',
    cardGradient: 'from-black to-neutral-800',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'No',
    atmLimit: 'Varies by plan',
    mobilePay: true,
    supportedAssets: 'Bitcoin, USDT, 70+ cryptocurrencies',
    kyc: 'Light',
    supportedCurrencies: ['USD']
  },
  {
    id: 'fiat24-card',
    name: 'Fiat24 Card',
    issuer: 'Fiat24',
    logo: getTwitterLogo('Fiat24Official'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: null,
    annualFee: 'Free',
    fxFee: 'Included in top-up fee (varies by tier)',
    perks: ['dApp', 'Swiss IBAN', 'Multicurrency', 'Tokenized fiat', 'SEPA/SWIFT payments', 'Apple Pay', 'Google Pay', 'Samsung Pay', 'Alipay', 'WeChat Pay'],
    signupBonus: 'None',
    custody: 'Hybrid',
    regions: 'Europe, Asia, Oceania, North America (specific countries with limits)',
    officialLink: 'https://fiat24.com/',
    cardGradient: 'from-violet-700 to-indigo-800',
    tierColor: 'text-white',
    metal: false, // virtual card only
    stakingRequired: "No",
    atmLimit: 'N/A (virtual card)',
    mobilePay: true,
    supportedAssets: 'ETH, USDC, USDC.e, USDT, ARB, LON, EUR24, CHF24, USD24, CNH24 (Arbitrum)',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'CHF', 'RMB', 'USD']
  },

  {
    id: 'thorwallet-card',
    name: 'THORWallet Card',
    issuer: 'THORWallet',
    logo: getTwitterLogo('thorwallet'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: null,
    annualFee: 'Free',
    fxFee: '1% (standard tier)',
    perks: ['Non-custodial', 'Swiss IBAN account', 'Multicurrency support (CHF, EUR, USD, RMB)', 'Tokenized fiat', 'No monthly fees', 'Apple Pay', 'Google Pay', 'SEPA/SWIFT payments'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: '100+ countries (Europe, Asia, Oceania, North America)',
    officialLink: 'https://www.thorwallet.org/',
    cardGradient: 'from-emerald-900 via-teal-800 to-slate-900',
    tierColor: 'text-white',
    metal: false, // virtual card only
    stakingRequired: "No",
    atmLimit: '$10,000/day (standard tier)',
    mobilePay: true,
    supportedAssets: 'Over 20,000 crypto tokens including ETH, USDC, USDT, BTC',
    kyc: 'Required',
    supportedCurrencies: ['CHF', 'EUR', 'USD', 'RMB']
  },
  {
    id: 'safepal-card',
    name: 'SafePal Card',
    issuer: 'SafePal',
    logo: getTwitterLogo('safepal'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: null,
    annualFee: 'Free',
    fxFee: '1%',
    perks: ['Non-custodial', 'Swiss IBAN account', 'Multicurrency', 'Lowest fees', 'Easy setup', 'Referral commission up to 40%', 'Apple Pay', 'Google Pay', 'Samsung Pay', 'Alipay', 'WeChat Pay', 'SEPA/SWIFT payments'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: '60+ countries (Europe, Asia, Oceania, North America)',
    officialLink: 'https://www.safepal.com/en/bank',
    cardGradient: 'from-blue-800 to-indigo-700',
    tierColor: 'text-white',
    metal: false, // virtual card only
    stakingRequired: "No",
    atmLimit: '€5,000/day',
    mobilePay: true,
    supportedAssets: 'Crypto across 40+ blockchains including ETH, USDC, USDT, BTC',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'CHF', 'RMB', 'USD']
  },
  {
    id: 'ur-card',
    name: 'UR Card',
    issuer: 'UR',
    logo: getTwitterLogo('UR_global'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: null,
    annualFee: 'Free',
    fxFee: 'Not specified',
    perks: ['Neobank', 'Swiss IBAN account', 'Multicurrency', 'Banking trust', 'Apple Pay', 'Google Pay', 'Samsung Pay', 'Alipay', 'WeChat Pay', 'SEPA/SWIFT payments'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: 'Europe, Asia, Oceania, North America',
    officialLink: 'https://ur.app/',
    cardGradient: 'from-black to-gray-900',
    tierColor: 'text-white',
    metal: false, // "virtual" card only
    stakingRequired: "No",
    atmLimit: '$11,611/day',
    mobilePay: true,
    supportedAssets: 'ETH, USDT, USDC, USDT0, WBTC, USDS, USDe, WETH, MNT, ENA, mETH, stETH, PYUSD, LINK, UNI, AAVE, LDO, PENDLE, ARB, POL, ENS, crvUSD, PAXG, XAUt; Fiat: EUR24, CHF24, USD24, CNH24, JPY24, SGD24, HKD24 (Arbitrum)',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'CHF', 'RMB', 'USD', 'SGD', 'HKD', 'JPY']
  },
  {
    id: 'savepay-card',
    name: 'SavePay Card',
    issuer: 'SavePay',
    logo: getTwitterLogo('savepay_org'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: null,
    annualFee: 'Free',
    fxFee: 'Not specified',
    perks: ['Non-custodial', 'Swiss IBAN', 'Free card', 'SEPA/SWIFT', 'Ultra low fees', 'Multicurrency', 'Referral commission 60%', 'Apple Pay', 'Google Pay', 'Samsung Pay', 'Alipay', 'WeChat Pay'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: 'Europe, Asia, Oceania, North America',
    officialLink: 'https://savepay.gitbook.io/savepay',
    cardGradient: 'from-green-300 to-green-400',
    tierColor: 'text-black',
    metal: false, // virtual card only
    stakingRequired: "No",
    atmLimit: '$109,384.22/month',
    mobilePay: true,
    supportedAssets: 'ETH, USDC, USDC.e, USDT, ARB, LON; Fiat: EUR24, CHF24, USD24, CNH24 (Arbitrum)',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'CHF', 'RMB', 'USD']
  },
  {
    id: 'imtoken-card',
    name: 'imToken Card',
    issuer: 'imToken',
    logo: getTwitterLogo('imTokenOfficial'),
    type: 'Debit',
    network: 'Mastercard',
    cashbackMax: null,
    annualFee: 'Free',
    fxFee: 'Low',
    perks: ['Non-custodial', 'Swiss IBAN', 'SEPA/SWIFT', 'Low fees', 'Multicurrency', 'High spend limits', 'Apple Pay', 'Google Pay', 'Samsung Pay', 'Alipay', 'WeChat Pay'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: 'Europe, Asia, Oceania, North America',
    officialLink: 'https://token.im/card?locale=en-US',
    cardGradient: 'from-blue-100 to-black',
    tierColor: 'text-black',
    metal: false, // virtual card only
    stakingRequired: "No",
    atmLimit: 'N/A (virtual card)',
    mobilePay: true,
    supportedAssets: 'ETH, USDC, USDT, WBTC (Arbitrum/ERC-20); Fiat: EUR24, CHF24, USD24, CNH24',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'CHF', 'RMB', 'USD']
  },
  {
    id: 'tokenpocket-card',
    name: 'TokenPocket Card',
    issuer: 'TokenPocket',
    logo: getTwitterLogo('TokenPocket_TP'),
    type: 'Debit',
    network: 'Visa',
    cashbackMax: null,
    annualFee: 'Free',
    fxFee: 'Not specified',
    perks: ['Non-custodial', 'Swiss IBAN', 'SEPA/SWIFT', 'Low fees', 'Multicurrency', 'High spend limits', 'Apple Pay', 'Google Pay', 'Samsung Pay', 'Alipay', 'WeChat Pay'],
    signupBonus: 'None',
    custody: 'Non-Custodial',
    regions: '30+ countries (Europe, Asia, Oceania, North America)',
    officialLink: 'https://card.tokenpocket.pro/?locale=en',
    cardGradient: 'from-neutral-950 to-black',
    tierColor: 'text-white',
    metal: false, // virtual card only
    stakingRequired: "No",
    atmLimit: '$100,000/month',
    mobilePay: true,
    supportedAssets: 'Any tokens on Arbitrum including ETH, USDC, USDT',
    kyc: 'Required',
    supportedCurrencies: ['EUR', 'CHF', 'RMB', 'USD']
  },
  {
    id: 'bitgetwallet-card',
    name: 'Bitget Wallet Card',
    issuer: 'Bitget Wallet',
    logo: getTwitterLogo('BitgetWallet'),
    type: 'Prepaid',
    network: 'Visa/Mastercard',
    cashbackMax: 2.2,
    annualFee: 'Free',
    fxFee: '1.7%',
    perks: ['0 Fees','Google-rate FX', '0 Fees on everyday spending (within monthly limit)', 'Spend crypto like cash, globally', 'Virtual Card & Physical Card supported',  'Apple Pay & Google Pay ready',  'Accepted worldwide (Visa, Mastercard network)',  'Fast & seamless KYC', 'Custom card designs & brand collaborations'],
    signupBonus: '$5',
    custody: 'Non-Custodial',
    regions: 'Global (Asia / LatAm / Europe, excluding US & sanctioned regions)',
    officialLink: 'https://web3.bitget.com/',
    cardGradient: 'from-cyan-500 to-blue-500',
    tierColor: 'text-white',
    metal: false,
    stakingRequired: 'None',
    atmLimit: '$10,000/day',
    mobilePay: true,
    supportedAssets: 'USDT, USDC',
    kyc: 'Light',
    supportedCurrencies: ['USD', 'Global']
  }
];
